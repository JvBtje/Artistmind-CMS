...
<tr><td><a href="#" onclick="parent.doselectparent(0,\'root\');return false">root</td></tr><tr><td>';
echo '<table>';
		$Laag++;
		$oldtext = $text;
		...
		echo '</table>';
	/*	
	$result = mysqli_query($link,"SELECT Id, Naam, TheOrder, MainId FROM usergroepen WHERE Language =".$_SESSION['Language']." AND Parent=0 ORDER BY TheOrder ");
	if (!$result) {
    	die('Query failed: ' . mysqli_error($link));
	}
	...
	while($row = mysqli_fetch_array($result)){
		...
		echo displaymenuparentselector($row["MainId"],0,"",array());
		echo '</div>';
	}*/
