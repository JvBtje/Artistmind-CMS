...
if ($_SESSION['newsession'] == false and $msgid != "new"){ 
	$msgid = intval($msgid);
		...
		$query = "SELECT Id, UserId, TheDate FROM reply WHERE Id =".$msgid;
...
if  ($acces == false or $found == false){
	...
	if ($msgid == "new"){
		...
		mysqli_query($link,"UPDATE reply SET Stat = 'normal', TheDate='$TheDate' WHERE Id = '$msgid '") or ($message = mysqli_error($link)); 
		echo '<?xml version="1.0" encoding="UTF-8"?><lijst>';
