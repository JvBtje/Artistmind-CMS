...
if (answer !=0) { 
	location = "indexadminnew.php?plugin=themaoverride&type=delete&sectie='.$sectie.'&Id=" + theId 
} 
}

 function changelanguase(veld, Id){
    if(veld == \'new\'){
 		 window.open(\'indexadminnew.php?plugin=themaoverride&type=new_Language&sectie='.$sectie.'&Id='.$MainId.'\',\'_self\',\'\',\'true\');
 	}else if(veld == \'delete\'){
	if (confirm(\'Are you sure to delete this language?\')) { 
 		 window.open(\'indexadminnew.php?plugin=themaoverride&type=delete_Language&sectie='.$sectie.'&Id=\'+'.$MainId.',\'_self\',\'\',\'true\');
		 }
 	}else if(veld == \'unknow\'){
  
 	}else{
	window.open(\'indexadminnew.php?plugin=themaoverride&language_id=\'+veld+\'&type=select&sectie='.$sectie.'&Id='.$MainId.'\',\'_self\',\'\',\'true\');
	} 
...
if ($type=="new_Language"){
		$output .= '		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			...
			<tr>
				...
				<td>';
				$output .='	<form action="indexadminnew.php?plugin=themaoverride&type=save_Language&sectie='.$sectie.'&Id='.$MainId.'" method="POST" name="form1" autocomplete="on">
	...
	$result = mysqli_query($link,"SELECT Themeurl FROM themeoverride WHERE IdGroup=".$IdGroup);
	if (!$result) {
    	die('Query failed: ' . mysqli_error($link));
	}
	...
	mysqli_query($link,"INSERT INTO themeoverride (IdGroup, Themeurl) VALUES ( '$Idgroup', '$Theme')")or  ($message = mysqli_error($link));
	...
	$output .= '<script type="text/javascript">setTimeout("window.open(\'indexadminnew.php?plugin=themaoverride&type=select&sectie='.$sectie.'&Id='.$MainIdGroup.'\',\'_self\',\'\',\'true\')", 0);</script>';
...
// checkt en veranderd de mainid als hij hetzelfde is als de id die verwijderd wordt zo blijft mainid gelinkt aan een id
	...
	mysqli_query($link,"DELETE FROM themeoverride WHERE IdGroup=".$IdGroup)or ($message = mysqli_error($link));
	$message = "";
	...
	array_push($_SESSION['Messages'], array("Date" => microtime(true), "html" => $message));
$output .= '<script type="text/javascript">setTimeout("window.open(\'indexadminnew.php?plugin=themaoverride&sectie='.$sectie.'\',\'_self\',\'\',\'true\')", 0);</script>';
	...
	if ($Id == "new"){
		mysqli_query($link,"INSERT INTO themeoverride (IdGroup, Themeurl) VALUES ( '-1', '$Theme')")or  ($message = mysqli_error($link));
		$Id = mysqli_insert_id($link);
		mysqli_query($link,"INSERT INTO groepen (Publish, Message, theDate, LastSaved, PublishDate, MainId, Naam, TheOrder, Language, Type, Parent, targetmainid) VALUES ('$Publish', '$Messagetrue','".date("Y-m-d H:i:s")."','".date("Y-m-d H:i:s")."','$DatumTijd', '-1', '$Naam', '$theorder','$language','themaoverride','$Parent', '$Id')")or  ($message = mysqli_error($link));
		$IdGroup = mysqli_insert_id($link);
		mysqli_query($link,"UPDATE groepen SET MainId = '$IdGroup' WHERE Id = '$IdGroup'") or ($message = mysqli_error($link));
		mysqli_query($link,"UPDATE themeoverride SET IdGroup = '$IdGroup' WHERE Id = '$Id'") or ($message = mysqli_error($link));
	}else{
		...
		mysqli_query($link,"UPDATE themeoverride SET Themeurl = '$Theme' WHERE IdGroup = '$IdGroup'") or ($message = mysqli_error($link)); 
	}
	...
	array_push($_SESSION['Messages'], array("Date" => microtime(true), "html" => $message));
	$output .= '<script type="text/javascript">setTimeout("window.open(\'indexadminnew.php?plugin=themaoverride&type=select&sectie='.$sectie.'&Id='.$MainId.'\',\'_self\',\'\',\'true\')", 0);</script>';
	
}else if($type=="delete"){
	$message = "Information deleted";
	mysqli_query($link,"DELETE FROM groepentousers WHERE GroepenMainId=".$MainId."" )or ($message = mysqli_error($link));
	mysqli_query($link,"DELETE FROM groepentousergroepen WHERE GroepenMainId=".$MainId."" )or ($message = mysqli_error($link));
	$result = mysqli_query($link,"SELECT Id, Language FROM groepen WHERE targetmainid=".$MainId);
	while($row = mysqli_fetch_array($result)){
		mysqli_query($link,"DELETE FROM themeoverride WHERE IdGroup=".$row["Id"])or ($message = mysqli_error($link));
	}
	...
	while($row = mysqli_fetch_array($result)){
		...
		$output .= '<form action="indexadminnew.php?plugin=themaoverride&type=save&sectie='.$sectie.'" method="POST" name="form1" autocomplete="on">
		<table>
		...
		<tr><td>Type</td><td>themeoverride</td></tr>
		...
		$output .='<tr><td>Thema </td><td><select name="Theme" id="Theme" onchange=changeval()>
				<option value="">Default Thema</option>';
				$dir ="./Themes/";
    $root = scandir($dir); 
 	$i=0;
	...
	if (is_file("$dir/$value") == true){
		...
		if ($i > 2){
			$output .= '<option '; if($row['Themeurl'] == $dir.$value.'/'){$output .= ' SELECTED ';}$output .= 'value="'.$dir.$value.'/">'.$value.'</option> ';
		}
	}
    } 
			$output .= '</select></td></tr>';
...
$output .= '</td>
		...
		$output .='</td></tr>
			...
			<tr><td>Thema </td><td><select name="Theme" id="Theme" onchange=changeval()>
				<option value="">Default Thema</option>';
				$dir ="./Themes/";
    $root = scandir($dir); 
 	$i=0;
	...
	if (is_file("$dir/$value") == true){
		...
		if ($i > 2){
			$output .= '<option '; if($row['Theme'] == $dir.$value.'/'){$output .= ' SELECTED ';}$output .= 'value="'.$dir.$value.'/">'.$value.'</option> ';
		}
	}
    } 
			$output .= '</select></td></tr>';
