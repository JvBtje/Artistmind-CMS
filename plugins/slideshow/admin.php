...
answer = confirm("Weet je zeker dat je deze slideshow wilt verwijderen.")

if (answer !=0) { 
	location = "indexadminnew.php?plugin=slideshow&type=delete&sectie='.$sectie.'&Id=" + theidgroup 
} 
}

 function changelanguase(veld, Id){
    if(veld == \'new\'){
 		 window.open(\'indexadminnew.php?plugin=slideshow&type=new_Language&sectie='.$sectie.'&Id='.$MainId.'\',\'_self\',\'\',\'true\');
 	}else if(veld == \'delete\'){
	if (confirm(\'Are you sure to delete this language?\')) { 
 		 window.open(\'indexadminnew.php?plugin=slideshow&type=delete_Language&sectie='.$sectie.'&Id=\'+'.$MainId.',\'_self\',\'\',\'true\');
		 }
 	}else if(veld == \'unknow\'){
  
 	}else{
	window.open(\'indexadminnew.php?plugin=slideshow&language_id=\'+veld+\'&type=select&sectie='.$sectie.'&Id='.$MainId.'\',\'_self\',\'\',\'true\');
	} 
...
if ($type=="new_Language"){
		$output .= '		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			...
			<tr>
				...
				<td>';
				$output .= '	<form action="indexadminnew.php?plugin=slideshow&type=save_Language&sectie='.$sectie.'&Id='.$MainId.'" method="POST" name="form1" autocomplete="on">
	...
	$result2 = mysqli_query($link,"SELECT Id, IdGroup, Theorder, Imageurl, Url FROM slides WHERE IdGroup=".$IdGroup." ORDER BY Theorder");
		...
		while($row2 = mysqli_fetch_array($result2)){
			...
			$Url = $row2['Url'];
			mysqli_query($link,"INSERT INTO slides (IdGroup, Theorder, Imageurl, Url) VALUES ('$Id','$Theorder', '$Imageurl','$Url')")or ($message = mysqli_error($link));
		}
	...
	$output .=  '<script type="text/javascript">setTimeout("window.open(\'indexadminnew.php?plugin=slideshow&type=select&sectie='.$sectie.'&Id='.$MainIdGroup.'\',\'_self\',\'\',\'true\')", 0);</script>';
...
// checkt en veranderd de mainid als hij hetzelfde is als de id die verwijderd wordt zo blijft mainid gelinkt aan een id
	...
	mysqli_query($link,"DELETE FROM slides WHERE IdGroup = ".$IdGroup )or ($message = mysqli_error($link));
	$message = "";
	...
	array_push($_SESSION['Messages'], array("Date" => microtime(true), "html" => $message));
$output .=  '<script type="text/javascript">setTimeout("window.open(\'indexadminnew.php?plugin=slideshow&sectie='.$sectie.'\',\'_self\',\'\',\'true\')", 0);</script>';
	...
	if ($Id == "new"){ 
		mysqli_query($link,"INSERT INTO groepen (Publish,Message, MainId, Naam, TheOrder, Language, Type, Parent, targetmainid, PublishDate, theDate, LastSaved) VALUES ( '$Publish','$Messagetrue', '-1', '$Naam', '$theorder','$language','slideshow','$Parent', '$Id','$DatumTijd', '".date("Y-m-d H:i:s")."', '".date("Y-m-d H:i:s")."')")or  ($message = mysqli_error($link));
	...
	// slaat images op
	mysqli_query($link,"DELETE FROM slides WHERE IdGroup=".$Id  )or ($message = mysqli_error($link));
	$totalimg = intval($_POST["totalslides"]);

	for ($i=0; $i<$totalimg;$i++){
		
		$imgname = $_POST["SlideImage".$i];
		$url = $_POST["SlideUrl".$i];
		...
		mysqli_query($link,"INSERT INTO slides ( IdGroup, Theorder, Imageurl, Url) VALUES ( '$Id', '$i','$imgname','$url')")or  ($message = mysqli_error($link));
...
mysqli_query($link,"DELETE FROM groepentousergroepen WHERE GroepenMainId=".$MainId."" )or ($message = mysqli_error($link));
	...
	array_push($_SESSION['Messages'], array("Date" => microtime(true), "html" => $message));
	$output .=  '<script type="text/javascript">setTimeout("window.open(\'indexadminnew.php?plugin=slideshow&type=select&sectie='.$sectie.'&Id='.$MainId.'\',\'_self\',\'\',\'true\')", 0);</script>';
}else if($type=="delete"){
$output .=  'Deleting...';
...
// checkt en veranderd de mainid als hij hetzelfde is als de id die verwijderd wordt zo blijft mainid gelinkt aan een id
	while($row = mysqli_fetch_array($result)){
		mysqli_query($link,"DELETE FROM groepentousergroepen WHERE GroepenMainId=".$MainId."" )or ($message = mysqli_error($link));
		mysqli_query($link,"DELETE FROM groepentousers WHERE GroepenMainId=".$MainId."" )or ($message = mysqli_error($link));
		mysqli_query($link,"DELETE FROM slides WHERE IdGroup = ".$row['Id'] )or ($message = mysqli_error($link));
...
$output .=  '</td><td><div id="buttonlayout"><h4><a href="#" onclick=" ConfirmDelete('.$MainId.','.$MainIdGroup.'); return false;">Delete</a></h4></div></td></tr></table>
		...
		$output .=  '<form action="indexadminnew.php?plugin=slideshow&type=save&sectie='.$sectie.'" method="POST" name="form1" autocomplete="on">
		<table>
		...
		$output .=  '</table>
		<div id="gallerybut"><a href="#" onClick="window.slideshows[\'main\'].slides.push(addslidetoshow(\'\',\'\',0,0));updateslides();return false"><img src="'.$_SESSION['Theme'].'/iconfilemanager/add.png" alt="add"></a><a href="#" onClick="selectallslides();return false"><img src="'.$_SESSION['Theme'].'/iconfilemanager/selectall.png" alt="select all"></a> <a href="#" onClick="deselectallslides();return false"><img src="'.$_SESSION['Theme'].'/iconfilemanager/deselectall.png" alt="deselect all"></a> <a href="#" onClick="deleteslide();return false"><img src="'.$_SESSION['Theme'].'/iconfilemanager/Delete.png" alt="delete files"></a></div>
		<div id="slideshow"></div><div id="slides">';
		$output .= '</div>';
...
$output .=  '<script language="javascript">createslideshow("main", \'slideshow\');</script>';
		while($row2 = mysqli_fetch_array($result2)){
			$output .=  '<script language="javascript">window.slideshows[\'main\'].slides.push(addslidetoshow(\''.$row2["Url"].'\',\''.$row2["Imageurl"].'\', \'0\', \'0\'));</script>';
		}
		$output .=  '<script language="javascript">updateslides();</script>';
		
		$output .= '
		<div id="buttonlayout"><h4><a href="javascript: submitform()">Save</a></h4></div>
...
$output .=  '</td>
		...
		$output .= '</table>';
		$output .=  '<div id="gallerybut"><a href="#" onClick="window.slideshows[\'main\'].slides.push(addslidetoshow(\'\',\'\',0,0));updateslides();return false"><img src="'.$_SESSION['Theme'].'/iconfilemanager/add.png" alt="add"></a><a href="#" onClick="selectallslides();return false"><img src="'.$_SESSION['Theme'].'/iconfilemanager/selectall.png" alt="select all"></a> <a href="#" onClick="deselectallslides();return false"><img src="'.$_SESSION['Theme'].'/iconfilemanager/deselectall.png" alt="deselect all"></a> <a href="#" onClick="deleteslide();return false"><img src="'.$_SESSION['Theme'].'/iconfilemanager/Delete.png" alt="delete files"></a></div>
		<div id="slideshow"></div><div id="slides">';
		...
		$output .= '</div>';
		$output .=  '<script language="javascript">createslideshow("main", \'slideshow\'); updateslides();</script>';
