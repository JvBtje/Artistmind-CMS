...
if (answer !=0) { 
	location = "indexadminnew.php?plugin=forum&type=delete&sectie='.$sectie.'&Id=" + theId 
} 
}

 function changelanguase(veld, Id){
    if(veld == \'new\'){
 		 window.open(\'indexadminnew.php?plugin=forum&type=new_Language&sectie='.$sectie.'&Id='.$MainId.'\',\'_self\',\'\',\'true\');
 	}else if(veld == \'delete\'){
	if (confirm(\'Are you sure to delete this language?\')) { 
 		 window.open(\'indexadminnew.php?plugin=forum&type=delete_Language&sectie='.$sectie.'&Id=\'+'.$MainId.',\'_self\',\'\',\'true\');
		 }
 	}else if(veld == \'unknow\'){
  
 	}else{
	window.open(\'indexadminnew.php?plugin=forum&language_id=\'+veld+\'&type=select&sectie='.$sectie.'&Id='.$MainId.'\',\'_self\',\'\',\'true\');
	} 
...
if ($type=="new_Language"){
		$output .= '		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			...
			<tr>
				...
				<td>';
				$output .='	<form action="indexadminnew.php?plugin=forum&type=save_Language&sectie='.$sectie.'&Id='.$MainId.'" method="POST" name="form1" autocomplete="on">
	...
	$result = mysqli_query($link,"SELECT Id, Language FROM forum WHERE MainId=".$MainId);
	if (!$result) {
    	die('Query failed: ' . mysqli_error($link));
	}
	...
	$result = mysqli_query($link,"SELECT Id, MainId, TheDate, MaxLijst FROM forum WHERE MainId=".$MainId." AND Language=". $_SESSION['Language']);
	if (!$result) {
    	die('Query failed: ' . mysqli_error($link));
	}
	...
	mysqli_query($link,"INSERT INTO forum (MainId, Language, TheDate, MaxLijst) VALUES ( '$MainId', '$newlanguage','$DatumTijd','$MaxLijst')")or  ($message = mysqli_error($link));
	...
	$output .= '<script type="text/javascript">setTimeout("window.open(\'indexadminnew.php?plugin=forum&type=select&sectie='.$sectie.'&Id='.$MainIdGroup.'\',\'_self\',\'\',\'true\')", 0);</script>';
...
// checkt en veranderd de mainid als hij hetzelfde is als de id die verwijderd wordt zo blijft mainid gelinkt aan een id
	...
	$result = mysqli_query($link,"SELECT Id, MainId FROM forum WHERE Language = ".$_SESSION['Language']." AND MainId=".$MainId );
		if (!$result) {
    	die('Query failed: ' . mysqli_error($link));
	}
// checkt en veranderd de mainid als hij hetzelfde is als de id die verwijderd wordt zo blijft mainid gelinkt aan een id
	while($row = mysqli_fetch_array($result)){
		if ($row['MainId'] == $row['Id']){
			...
			$MainId = $row['MainId'];
			$changed = false;
			$result2 = mysqli_query($link,"SELECT Id, MainId FROM forum WHERE MainId=".$row['MainId'] );
			if (!$result2) {
    			die('Query failed: ' . mysqli_error($link));
			}
			...
			while($row2 = mysqli_fetch_array($result2)){
				...
				mysqli_query($link,"UPDATE forum SET MainId = '$MainId' WHERE MainId = '$oldMainId '") or ($message = mysqli_error($link));
	...
	mysqli_query($link,"DELETE FROM forum WHERE Language = ".$_SESSION['Language']." AND MainId=".$MainId  )or ($message = mysqli_error($link));
	$message = "";
	...
	array_push($_SESSION['Messages'], array("Date" => microtime(true), "html" => $message));
$output .= '<script type="text/javascript">setTimeout("window.open(\'indexadminnew.php?plugin=forum&sectie='.$sectie.'\',\'_self\',\'\',\'true\')", 0);</script>';
	...
	$MaxLijst = intval($_POST["ShowLijst"]);
	...
	if ($Id == "new"){
		mysqli_query($link,"INSERT INTO forum ( MainId, Language, TheDate, MaxLijst) VALUES ( '-1', '$language','$DatumTijd','$MaxLijst')")or  ($message = mysqli_error($link));
		$Id = mysqli_insert_id($link);
		mysqli_query($link,"UPDATE forum SET MainId = '$Id' WHERE Id = '$Id'") or ($message = mysqli_error($link)); 
		mysqli_query($link,"INSERT INTO groepen (Publish, Message, theDate, LastSaved, PublishDate, MainId, Naam, TheOrder, Language, Type, Parent, targetmainid) VALUES ('$Publish', '$Messagetrue','".date("Y-m-d H:i:s")."','".date("Y-m-d H:i:s")."','$DatumTijd', '-1', '$Naam', '$theorder','$language','forum','$Parent', '$Id')")or  ($message = mysqli_error($link));
		...
		mysqli_query($link,"UPDATE forum SET TheDate = '$DatumTijd', MaxLijst= '$MaxLijst' WHERE Id = '$Id'") or ($message = mysqli_error($link)); 
	}
	...
	array_push($_SESSION['Messages'], array("Date" => microtime(true), "html" => $message));
	$output .= '<script type="text/javascript">setTimeout("window.open(\'indexadminnew.php?plugin=forum&type=select&sectie='.$sectie.'&Id='.$MainId.'\',\'_self\',\'\',\'true\')", 0);</script>';
	
}else if($type=="delete"){
$message = "Information deleted";
mysqli_query($link,"DELETE FROM groepentousers WHERE GroepenMainId=".$MainId."" )or ($message = mysqli_error($link));
mysqli_query($link,"DELETE FROM groepentousergroepen WHERE GroepenMainId=".$MainId."" )or ($message = mysqli_error($link));
mysqli_query($link,"DELETE FROM forum WHERE MainId=$MainId")or ($message = mysqli_error($link));
...
$output .= '<script type="text/javascript">setTimeout("window.open(\'indexadminnew.php?plugin=forum&sectie='.$sectie.'\',\'_self\',\'\',\'true\')", 0);</script>';
	...
	$result = mysqli_query($link,"SELECT Id, MaxLijst FROM forum WHERE MainId=".$MainId." AND Language=". $_SESSION['Language']);
	if (!$result) {
    	die('Query failed: ' . mysqli_error($link));
	}
	...
	while($row = mysqli_fetch_array($result)){
		...
		$output .= '<form action="indexadminnew.php?plugin=forum&type=save&sectie='.$sectie.'" method="POST" name="form1" autocomplete="on">
		<table>
		...
		$output .= '
			<tr><td>Maximum Viewable</td><td><input type="text" name="ShowLijst" value="'.$row["MaxLijst"].'" size="5" border="0" onchange="changeval();"></td></tr>
			';
		$output .= '
