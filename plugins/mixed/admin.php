...
if (answer !=0) { 
	location = "indexadminnew.php?plugin=mixed&type=delete&Id=" + theId + "&sectie='.$sectie.'";
} 
}

 function changelanguase(veld, Id){
    if(veld == \'new\'){
 		 window.open(\'indexadminnew.php?plugin=mixed&type=new_Language&sectie='.$sectie.'&Id='.$MainId.'\',\'_self\',\'\',\'true\');
 	}else if(veld == \'delete\'){
	if (confirm(\'Are you sure to delete this language?\')) { 
 		 window.open(\'indexadminnew.php?plugin=mixed&type=delete_Language&sectie='.$sectie.'&delete_Language_Id=\'+Id,\'_self\',\'\',\'true\');
		 }
 	}else if(veld == \'unknow\'){
  
 	}else{
	window.open(\'indexadminnew.php?plugin=mixed&language_id=\'+veld+\'&sectie='.$sectie.'&type=select&Id='.$MainId.'\',\'_self\',\'\',\'true\');
	} 
...
if ($type=="new_Language"){
				$output .= '	<form action="indexadminnew.php?plugin=mixed&type=save_Language&sectie='.$sectie.'&Id='.$MainId.'" method="POST" name="form1" autocomplete="on">
	...
	$result = mysqli_query($link,"SELECT Publish, Naam, Message, TheOrder, MainId, Language, Parent, Type, LastSaved, PublishDate FROM groepen WHERE  MainId=".$MainId." AND Language=". $_SESSION['Language'] );
	if (!$result) {
    	die('Query failed: ' . mysqli_error($link));
	}
...
// Voor zoekt iso6931 codes op voor vertaling
	...
	mysqli_query($link,"INSERT INTO groepen (Publish, Message, MainId, Naam, TheOrder, Language, Parent, Type, LastSaved, PublishDate, basedocument) VALUES ('$Publish','$Messagetrue','$MainId', '$Naam','$TheOrder','$newlanguage',$Parent,'$Type','$LastSaved','$PublishDate',1 )")or ($message = mysqli_error($link));
	...
	$output .= '<script type="text/javascript">setTimeout("window.open(\'indexadminnew.php?plugin=mixed&type=select&sectie='.$sectie.'&Id='.$MainId.'\',\'_self\',\'\',\'true\')", 0);</script>';
...
array_push($_SESSION['Messages'], array("Date" => microtime(true), "html" => $message));
$output .= '<script type="text/javascript">setTimeout("window.open(\'indexadminnew.php?plugin=mixed&sectie='.$sectie.'\',\'_self\',\'\',\'true\')", 0);</script>';
	...
	if ($Id == "new"){
		mysqli_query($link,"INSERT INTO groepen (Publish,Message, MainId, Naam, TheOrder, Language, Type, Parent, LastSaved, PublishDate, basedocument) VALUES ('$Publish','$Messagetrue', '-1', '$Naam', '$theorder','$language','mixed','$Parent','$LastSaved','$DatumTijd',1 )")or  ($message = mysqli_error($link));
		$Id = mysqli_insert_id($link);
		mysqli_query($link,"UPDATE groepen SET MainId = '$Id' WHERE Id = '$Id'") or ($message = mysqli_error($link)); 
	}else{
		mysqli_query($link,"UPDATE groepen SET Publish = '$Publish', Message = '$Messagetrue', Naam = '$Naam', Parent = '$Parent', LastSaved = '$LastSaved', PublishDate = '$DatumTijd' WHERE Id = '$Id'") or ($message = mysqli_error($link)); 
		//array_push($_SESSION['Messages'], array("Date" => microtime(true), "html" => "UPDATE groepen SET Publish = '$Publish', Message = '$Messagetrue', Naam = '$Naam', Parent = '$Parent',LastSaved = '$LastSaved', PublishDate='$DatumTijd' WHERE Id = '$Id'"));
	}
...
$result = mysqli_query($link,"SELECT Id, Language FROM groepen WHERE MainId=".$MainId);
	...
	$result = mysqli_query($link,"SELECT Id, Publish, Naam, TheOrder,Message, Parent, Type, PublishDate FROM groepen WHERE MainId=".$MainId." AND Language=". $_SESSION['Language']);
	if (!$result) {
    	die('Query failed: ' . mysqli_error($link));
	}
	...
	while($row = mysqli_fetch_array($result)){
		...
		$output .= '<form action="indexadminnew.php?plugin=mixed&type=save&sectie='.$sectie.'" method="POST" name="form1" autocomplete="on">
		<table>
...
$output .= '</td>
		...
		<tr><td>Naam page</td><td><input type="text" name="Naam" value="" size="75" border="0" onchange="changeval();"></td></tr>	
