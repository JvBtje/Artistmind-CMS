...
if (answer !=0) { 
	location = "indexadminnew.php?plugin=bar&type=delete&Id=" + theId + "&sectie='.$sectie.'";
} 
}

 function changelanguase(veld, Id){
    if(veld == \'new\'){
 		 window.open(\'indexadminnew.php?plugin=bar&type=new_Language&sectie='.$sectie.'&Id='.$MainId.'\',\'_self\',\'\',\'true\');
 	}else if(veld == \'delete\'){
	if (confirm(\'Are you sure to delete this language?\')) { 
 		 window.open(\'indexadminnew.php?plugin=bar&type=delete_Language&sectie='.$sectie.'&delete_Language_Id=\'+Id,\'_self\',\'\',\'true\');
		 }
 	}else if(veld == \'unknow\'){
  
 	}else{
	window.open(\'indexadminnew.php?plugin=bar&language_id=\'+veld+\'&sectie='.$sectie.'&type=select&Id='.$MainId.'\',\'_self\',\'\',\'true\');
	} 
...
if ($type=="new_Language"){
				$output .= '	<form action="indexadminnew.php?plugin=bar&type=save_Language&sectie='.$sectie.'&Id='.$MainId.'" method="POST" name="form1" autocomplete="on">
...
// Voor zoekt iso6931 codes op voor vertaling
	...
	$output .= '<script type="text/javascript">setTimeout("window.open(\'indexadminnew.php?plugin=bar&type=select&sectie='.$sectie.'&Id='.$MainId.'\',\'_self\',\'\',\'true\')", 0);</script>';
...
array_push($_SESSION['Messages'], array("Date" => microtime(true), "html" => $message));
$output .= '<script type="text/javascript">setTimeout("window.open(\'indexadminnew.php?plugin=bar&sectie='.$sectie.'\',\'_self\',\'\',\'true\')", 0);</script>';
	...
	if ($Id == "new"){
		mysqli_query($link,"INSERT INTO groepen (Publish,Message, MainId, Naam, TheOrder, Language, Type, Parent, LastSaved, PublishDate, basedocument) VALUES ('$Publish','$Messagetrue', '-1', '$Naam', '$theorder','$language','bar','$Parent','$LastSaved','$DatumTijd',1 )")or  ($message = mysqli_error($link));
		$Id = mysqli_insert_id($link);
...
$result = mysqli_query($link,"SELECT Id, Language FROM groepen WHERE MainId=".$MainId);
	...
	while($row = mysqli_fetch_array($result)){
		...
		$output .= '<form action="indexadminnew.php?plugin=bar&type=save&sectie='.$sectie.'" method="POST" name="form1" autocomplete="on">
		<table>
