...
if (answer !=0) { 
	location = "indexstandalone.php?plugin=menumemberedit&type=delete&Id=" + theId 
} 
}

 function changelanguase(veld, Id){
    if(veld == \'new\'){
 		 window.open(\'indexstandalone.php?plugin=menumemberedit&type=new_Language&Id='.$MainId.'\',\'_self\',\'\',\'true\');
 	}else if(veld == \'delete\'){
	if (confirm(\'Are you sure to delete this language?\')) { 
 		 window.open(\'indexstandalone.php?plugin=menumemberedit&type=delete_Language&delete_Language_Id=\'+Id,\'_self\',\'\',\'true\');
		 }
 	}else if(veld == \'unknow\'){
  
 	}else{
	window.open(\'indexstandalone.php?plugin=menumemberedit&language_id=\'+veld+\'&type=select&Id='.$MainId.'\',\'_self\',\'\',\'true\');
	} 
...
if ($type=="new_Language"){
		$output .= '		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			...
			<tr>
				...
				<td>';
				$output .='	<form action="indexstandalone.php?plugin=menumemberedit&type=save_Language&Id='.$MainId.'" method="POST" name="form1" autocomplete="on">
				<input type="hidden" name="changed" id="changed" value="false"><select name="language" id="language" onchange=changeval()>';

	$result = mysqli_query($link,"SELECT Id, Language FROM menumember WHERE MainId=".$MainId);
	if (!$result) {
	...
	$found = false;
	if ($newlanguage <> ""){
	$result = mysqli_query($link,"SELECT Id FROM menumember WHERE Language =".$newlanguage ." AND MainId=".$MainId );
	if (!$result) {
    	die('Query failed: ' . mysqli_error($link));
	}
	...
	$result = mysqli_query($link,"SELECT HasSubMenu, NumCol, ColWidth,ColHeigth, Largtext1bg, Largtext2bg, Largtext3bg,Largtext4bg,Largtext5bg, Largtext6bg,LargText, Largtext2, Largtext3,Largtext4,Largtext5, Largtext6, Naam, TheOrder, MainId, Url, Window FROM menumember WHERE  MainId=".$MainId." AND Language=". $_SESSION['Language'] );
	if (!$result) {
    	die('Query failed: ' . mysqli_error($link));
	}
	...
	$result = mysqli_query($link,"SELECT Naam, TheOrder, MainId, Url, Window FROM menumember WHERE  TheOrder=".$TheOrder." AND Language=". $newlanguage );
	if (!$result) {
    	die('Query failed: ' . mysqli_error($link));
	}
	...
	while($row = mysqli_fetch_array($result)){
		...
		$result2 = mysqli_query($link,"SELECT TheOrder FROM menumember ORDER BY TheOrder LIMIT 0,1");
		if (!$result2) {
    		die('Query failed: ' . mysqli_error($link));
		}
	...
	$output .= '<script type="text/javascript">setTimeout("window.open(\'indexstandalone.php?plugin=menumemberedit&type=select&sectie='.$sectie.'&Id='.$MainId.'\',\'_self\',\'\',\'true\')", 0);</script>';
	
} else if ($type=="delete_Language"){
	$result = mysqli_query($link,"SELECT Id, MainId FROM menumember WHERE Id=".$_GET['delete_Language_Id'] );
		if (!$result) {
    	die('Query failed: ' . mysqli_error($link));
	}
// checkt en veranderd de mainid als hij hetzelfde is als de id die verwijderd wordt zo blijft mainid gelinkt aan een id
	while($row = mysqli_fetch_array($result)){
		if ($row['MainId'] == $row['Id']){
			$MainId = $row['MainId'];
			$changed = false;
			$result2 = mysqli_query($link,"SELECT Id, MainId FROM menumember WHERE MainId=".$row['MainId'] );
			if (!$result2) {
    			die('Query failed: ' . mysqli_error($link));
			}
			...
			while($row2 = mysqli_fetch_array($result2)){
				...
				mysqli_query($link,"UPDATE menumember SET MainId = '$MainId' WHERE Id = '$Id'") or ($message = mysqli_error($link)); 
			}
		} 
	}
	$message = "";
		mysqli_query($link,"DELETE FROM menumember WHERE Id=".$_GET['delete_Language_Id'])or ($message = mysqli_error($link));
		if ($message == ""){	
...
array_push($_SESSION['Messages'], array("Date" => microtime(true), "html" => $message));
$output .= '<script type="text/javascript">setTimeout("window.open(\'indexstandalone.php?plugin=menumemberedit&sectie='.$sectie.'\',\'_self\',\'\',\'true\')", 0);</script>';
	...
	$result = mysqli_query($link,"SELECT TheOrder FROM menumember ORDER BY TheOrder LIMIT 0,1");
	if (!$result) {
    	die('Query failed: ' . mysqli_error($link));
	}
	...
	if ($Id == "new"){
		mysqli_query($link,"INSERT INTO menumember ( HasSubMenu, TheOrder, MainId, Naam,  Language, Url, Window, Largtext1bg, Largtext2bg, Largtext3bg, Largtext4bg, Largtext5bg, Largtext6bg, LargText, Largtext2, Largtext3,Largtext4,Largtext5, Largtext6,NumCol,ColWidth,ColHeigth) VALUES ( '$HasSubMenu','$theorder', '-1', '$Naam', '$language','$Url','$Window','$bg[1]','$bg[2]','$bg[3]','$bg[4]','$bg[5]','$bg[6]','$LargText[1]','$LargText[2]','$LargText[3]','$LargText[4]','$LargText[5]','$LargText[6]','$NumCols','$ColWidth','$ColHeigth')")or  ($message = mysqli_error($link));
		$Id = mysqli_insert_id($link);
		mysqli_query($link,"UPDATE menumember SET MainId = '$Id' WHERE Id = '$Id'") or ($message = mysqli_error($link)); 
	}else{
		mysqli_query($link,"UPDATE menumember SET HasSubMenu='$HasSubMenu', Naam = '$Naam', Url = '$Url', Window='$Window', Largtext1bg = '$bg[1]',Largtext2bg = '$bg[2]',Largtext3bg = '$bg[3]',Largtext4bg = '$bg[4]',Largtext5bg = '$bg[5]',Largtext6bg = '$bg[6]',LargText = '$LargText[1]',Largtext2 = '$LargText[2]',Largtext3 = '$LargText[3]',Largtext4 = '$LargText[4]',Largtext5 = '$LargText[5]',Largtext6 = '$LargText[6]',NumCol = '$NumCols',ColWidth = '$ColWidth',ColHeigth = '$ColHeigth'WHERE Id = '$Id'") or ($message = mysqli_error($link)); 
		
	}
	$result = mysqli_query($link,"SELECT MainId FROM menumember WHERE Id=".$Id);
	if (!$result) {
    	die('Query failed: ' . mysqli_error($link));
	}
...
mysqli_query($link,"DELETE FROM menumember WHERE MainId=$MainId")or ($message = mysqli_error($link));
...
$result = mysqli_query($link,"SELECT Id, Language FROM menumember WHERE MainId=".$MainId);
	...
	$output .= '</select></td><td><div id="buttonlayout"><a href="indexstandalone.php?plugin=menumemberedit&type=new"><h4>new</h4></a></div></td><td><div id="buttonlayout"><a href="#" onclick=" ConfirmDelete('.$MainId.'); return false;"><h4>Delete</h4></a></div></td></tr></table>
				</td>
	...
	$result = mysqli_query($link,"SELECT Id, HasSubMenu, NumCol,ColWidth,ColHeigth, Largtext1bg, Largtext2bg, Largtext3bg,Largtext4bg,Largtext5bg, Largtext6bg,LargText, Largtext2, Largtext3,Largtext4,Largtext5, Largtext6, Naam, TheOrder, Url,Window FROM menumember WHERE MainId=".$MainId." AND Language=". $_SESSION['Language']);
	if (!$result) {
    	die('Query failed: ' . mysqli_error($link));
	}
	...
	while($row = mysqli_fetch_array($result)){
		...
		$output .= '<form action="indexstandalone.php?plugin=menumemberedit&type=save" method="POST" name="form1" autocomplete="on">
		<table>
		...
		<tr><td><div id="buttonlayout"><a href="javascript: submitform()"><h4>Save</h4></a></div></td></td><td></td></tr></table>
...
$output .= '		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				...
				<td><div id="buttonlayout"><a href="indexstandalone.php?plugin=menumemberedit&type=new"><h4>new</h4></a></div></td>
			</tr>
			<tr>
...
$output .= '<table width="100%" border="0" cellspacing="0" cellpadding="0">	<tr><td>';		
		$output .='</td><td><div id="buttonlayout"><a href="indexstandalone.php?plugin=menumemberedit&type=new"><h4>new</h4></a></div></td></tr><tr><td>';
			$output .='</td></tr></table>';
}
}else{
